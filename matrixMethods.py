...
Computational Physics Matrix Methods
"""
...
def MatrixMultiplication( matrix1 , matrix2 ):
    ...
    if matrix2.size == matrix2.shape[0]:                  # Then second matrix is a vector, different multiplication
        matrix3 = np.zeros([matrix2.size])
        for row in range(matrix1.shape[0]):           
            sumRow = 0
            for element in range(matrix1.shape[1]):  
                sumRow +=  matrix1[row][element] * matrix2[element]   #Elements in A_mi and B_in summed over i's
            matrix3[row] = sumRow    
    else:
        print(" else in Matrix Multip")
        ...
        for row in range(matrix1.shape[0]):
            for column in range(matrix2.shape[1]):
                sum_coln = 0
                for element in range(matrix1.shape[1]):  
    ...
    for m in range(matrix.shape[0]):            # Looping through columns m
        ...
        for n in range(matrix.shape[1]):        # Looping through rows for a column m
            if n!=m:                            # Making sure we dont change A_mm
                ...
                vector[n] += vector[m] * -matrix[n][m]       # V_n = V_n + V_m * A_nm
                matrix[n] += matrix[m] * -matrix[n][m]       # A_mn = 0
...
if __name__ == "__main__":
    ...
    aMatrix = np.array( [[ 2., 1., 9., 3.] ,
                         [-3., 0.,-2., 2.] ,
                         [ 1., 3.,-5.,-1.] ,
                         [-6., 4.,-1.,-4.]] )
    
    bVector = np.array( [ -6., 3., 1.,22.] )
